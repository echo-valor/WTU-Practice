...
public class EditStudentInfo  extends JFrame implements ActionListener,ItemListener  {
	...
	private JButton Save,Cancel,Return, Save1;//1.保存按钮，2.取消按钮,3.返回按钮
	...
	public EditStudentInfo(String sid, String sname, String ssex, String sbirthday, String sprovince, String hobby, String sphone) {
	...
	private void EditStudentInfoInterface(String sid, String sname, String ssex, String sbirthday, String sprovince, String hobby, String sphone) {
		...
		//int[] hobbyFlag = new int[12];
			/*for(int i=0; i<hobbys.length; i++){
				if(hobbys[i].equals("足球")){
					hobby1.setSelected(true);
				}

				if(hobbys[i].equals("篮球")){
					hobby2.setSelected(true);
				}

				if(hobbys[i].equals("乒乓球")){
					hobby3.setSelected(true);
				}
				if(hobbys[i].equals("看电影")){
					hobby4.setSelected(true);
				}

				if(hobbys[i].equals("羽毛球")){
					hobby5.setSelected(true);
				}

				if(hobbys[i].equals("游泳")){
					hobby6.setSelected(true);
				}

				if(hobbys[i].equals("旅游")){
					hobby7.setSelected(true);
				}

				if(hobbys[i].equals("唱歌")){
					hobby8.setSelected(true);
				}

				if(hobbys[i].equals("拍照")){
					hobby9.setSelected(true);
				}

				if(hobbys[i].equals("打游戏")){
					hobby10.setSelected(true);
				}

				if(hobbys[i].equals("逛街")){
					hobby11.setSelected(true);
				}

				if(hobbys[i].equals("看书")){
					hobby12.setSelected(true);
				}
		...
		Save1.setUI(new BEButtonUI()
				.setNormalColor(BEButtonUI.NormalColor.blue));//设置按钮的背景颜色
		Save1.setBounds(new Rectangle(250, 520, 70,30));
		Save1.setFont(font2);
		...
		Save1.addMouseListener(new MouseListener() {
...
//				System.out.println(hobby.toString());
				...
				boolean flag = addstudentdao.updatestudentinformation(student);//将数据插入数据库
				if(flag == true) {
